...
pub struct ShadowQuicClientCfg {
    ...
    /// If set to 0, port hopping is disabled.
    /// If set to a positive value, the client will periodically hop to a new server port.
    /// The actual hop interval:
    ///   - If max_interval < 5, uses fixed 5 seconds
    ///   - If max_interval >= 5, uses random between 5 seconds and max_interval
    /// This helps evade UDP blocking and port-based traffic classification.
    /// Recommended value: 10-60 seconds.
    #[serde(default)]
    pub port_hop_interval: u64,

    /// Server port range for port hopping.
    /// Format: "start-end" (e.g., "50000-60000").
    /// When port hopping is enabled, the client will randomly connect to a port
    /// within this range instead of the default port specified in addr.
    /// The server should listen on multiple ports or use iptables/nftables to redirect.
    #[serde(default)]
