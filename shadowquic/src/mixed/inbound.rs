...
impl Inbound for MixedServer {
    async fn accept(&mut self) -> Result<ProxyRequest, SError> {
        ...
        let fut = async {
            ...
            const MIN_PROTOCOL_SNIFF: usize = 8;

            while filled < MIN_PROTOCOL_SNIFF {
                let n = stream.read(&mut sniff[filled..]).await?;
                if n == 0 {
                ...
                if Self::looks_like_socks5(&sniff[..filled]) {
        ...
        timeout(HANDSHAKE_TIMEOUT, fut)
            .await
            .map_err(|_| SError::SocksError("mixed inbound handshake timeout".into()))?
    }
}
